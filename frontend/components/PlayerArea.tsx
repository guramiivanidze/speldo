  ...
  return (
    <div
      className={`
        ...
        ${isCurrentTurn
          ? 'border border-amber-500/60 turn-pulse'
    ...
    >
          ...
          {GEM_COLORS.map((color) => {
            ...
            return (
              <div key={color} className="flex flex-col items-center gap-0.5">
                <div
                  className="gem-orb"
                  style={{ width: 14, height: 14, background: GEM_DOT_STYLE[color] }}
                />
      ...
      {/* ── Reserved cards (my panel) ───────────────── */}
      {isMe && player.reserved_card_ids.length > 0 && (
        <div>
          <button
            className="flex items-center gap-1.5 text-[10px] text-slate-400 font-semibold uppercase tracking-wider mb-1.5 hover:text-slate-200 transition-colors"
            onClick={() => setShowReserved(v => !v)}
          >
            <span>Reserved ({player.reserved_card_ids.length}/3)</span>
            <span className="text-[8px]">{showReserved ? '▲' : '▼'}</span>
          </button>
          {showReserved && (
            <div className="flex gap-1.5 flex-wrap">
